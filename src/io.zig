...
// braingang patch: omitted Windows backend (not vendored). See UPSTREAM.md.

pub const IO = switch (builtin.target.os.tag) {
    .linux => IO_Linux,
    .macos, .tvos, .watchos, .ios => IO_Darwin,
    else => @compileError("tigerbeetle_io: platform unsupported (Linux io_uring or Darwin kqueue only)"),
};
