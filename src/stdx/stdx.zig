...
// braingang port: removed pub-const exports for stdx submodules that
// `tigerbeetle_io` consumers do not reference. Kept: windows submodule, and
// the three time_units types (Instant, Duration, InstantUnix). See UPSTREAM.md
// (Port divergence from upstream stdx).
...
// braingang port: removed `test "hash_inline"` that imported
// `testing/low_level_hash_vectors.zig` (not vendored). See UPSTREAM.md.
...
// braingang port: removed `EnumUnionType` helper (upstream lines 759-789).
// Body used `@Type(.{ .@"union" = ...})` which Zig 0.16 split into a new
// `@Union(layout, tag_type, names, types, attrs)` builtin with a different
// arg shape. The helper is unreferenced by the IO closure, so it is dropped
// rather than ported. See UPSTREAM.md.
...
// braingang port: removed `test "ByteSize.parse_flag_value"` that called
// `Flags.parse_flag_value_fuzz` — `Flags` (flags.zig) is not vendored. See
// UPSTREAM.md.
...
// braingang port: removed `const snap = Snap.snap_fn(...)` plus the two
// `test fastrange` / `test "fastrange not modulo"` blocks; both used `Snap`
// and `PRNG`, which are not vendored. See UPSTREAM.md.
...
comptime {
    // braingang port: comptime _ = @import imports for stdx submodules that
    // have been removed from this port. Kept: time_units, windows. The
    // self-reference to "stdx.zig" was dropped as redundant.
